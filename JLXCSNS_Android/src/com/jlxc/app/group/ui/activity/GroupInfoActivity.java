...
//		intent.getIntExtra(INTENT_KEY, 0);
				...
				new LoadDataHandler<String>() {
					...
					public void onSuccess(JSONObject jsonResponse, String flag) {
						...
						if (status == JLXCConst.STATUS_FAIL) {
							ToastUtil.show(GroupInfoActivity.this, jsonResponse.getString(JLXCConst.HTTP_MESSAGE));
						}
					}
	...
	private void handleResult(JSONObject result) {
		...
		for (int i = 0; i < members.size(); i++) {
			...
			ImageLoader.getInstance().displayImage(JLXCConst.ATTACHMENT_ADDR+member.getHead_sub_image(), memberImageView, options);
		}
